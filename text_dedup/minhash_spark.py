#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Author  : Priyam Mehya (priyammehta@gmail.com)

"""
This is an experimental version of MinHashLSH using PySpark. It was originally designed for The Stack Dataset.
The code has been refactored for Sangraha Dataset
"""
...
if __name__ == "__main__":  # pragma: no cover
    ...
    cluster_df = df.join(components, on="__id__", how="left").cache()
    ...
    df = cluster_df.filter(F.col("__component__").isNull())
    df = df.drop("__id__", "__component__").cache()
    ...
    cluster_df.repartition(max(1, DATA_SIZE // WRITE_ROWS)).withColumn("__pid__", F.spark_partition_id()).write.partitionBy(
        "__pid__"
